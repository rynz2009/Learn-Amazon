...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.niche.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.niches.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="niche">{{ trans('cruds.niche.fields.niche') }}</label>
                <input class="form-control {{ $errors->has('niche') ? 'is-invalid' : '' }}" type="text" name="niche" id="niche" value="{{ old('niche', '') }}">
                @if($errors->has('niche'))
