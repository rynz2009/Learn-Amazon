...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.detailTransaksi.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.detail-transaksis.update", [$detailTransaksi->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_transaksi') ? 'is-invalid' : '' }}" name="kode_transaksi_id" id="kode_transaksi_id" required>
                    @foreach($kode_transaksis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kode_transaksi_id') ? old('kode_transaksi_id') : $detailTransaksi->kode_transaksi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_produks') ? 'is-invalid' : '' }}" name="kode_produks[]" id="kode_produks" multiple required>
                    @foreach($kode_produks as $id => $kode_produk)
                        <option value="{{ $id }}" {{ (in_array($id, old('kode_produks', [])) || $detailTransaksi->kode_produks->contains($id)) ? 'selected' : '' }}>{{ $kode_produk }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('keterangan_desain') ? 'is-invalid' : '' }}" name="keterangan_desain" id="keterangan_desain">{{ old('keterangan_desain', $detailTransaksi->keterangan_desain) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="text" name="size" id="size" value="{{ old('size', $detailTransaksi->size) }}" required>
                @if($errors->has('size'))
                    <div class="invalid-feedback">
                        {{ $errors->first('size') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('warna') ? 'is-invalid' : '' }}" type="text" name="warna" id="warna" value="{{ old('warna', $detailTransaksi->warna) }}" required>
                @if($errors->has('warna'))
                    <div class="invalid-feedback">
                        {{ $errors->first('warna') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah') ? 'is-invalid' : '' }}" type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $detailTransaksi->jumlah) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('harga') ? 'is-invalid' : '' }}" type="number" name="harga" id="harga" value="{{ old('harga', $detailTransaksi->harga) }}" step="0.01" required>
                @if($errors->has('harga'))
                    <div class="invalid-feedback">
                        {{ $errors->first('harga') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_hpps') ? 'is-invalid' : '' }}" name="kode_hpps[]" id="kode_hpps" multiple required>
                    @foreach($kode_hpps as $id => $kode_hpp)
                        <option value="{{ $id }}" {{ (in_array($id, old('kode_hpps', [])) || $detailTransaksi->kode_hpps->contains($id)) ? 'selected' : '' }}>{{ $kode_hpp }}</option>
                    @endforeach
                </select>
