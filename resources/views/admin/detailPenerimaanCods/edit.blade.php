...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.detailPenerimaanCod.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.detail-penerimaan-cods.update", [$detailPenerimaanCod->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_pcods') ? 'is-invalid' : '' }}" name="kode_pcods[]" id="kode_pcods" multiple required>
                    @foreach($kode_pcods as $id => $kode_pcod)
                        <option value="{{ $id }}" {{ (in_array($id, old('kode_pcods', [])) || $detailPenerimaanCod->kode_pcods->contains($id)) ? 'selected' : '' }}>{{ $kode_pcod }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('awb') ? 'is-invalid' : '' }}" type="text" name="awb" id="awb" value="{{ old('awb', $detailPenerimaanCod->awb) }}" required>
                @if($errors->has('awb'))
                    <div class="invalid-feedback">
                        {{ $errors->first('awb') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nilai_awb') ? 'is-invalid' : '' }}" type="number" name="nilai_awb" id="nilai_awb" value="{{ old('nilai_awb', $detailPenerimaanCod->nilai_awb) }}" step="0.01" required>
