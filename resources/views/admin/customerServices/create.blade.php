...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.customerService.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.customer-services.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="kode_cs">{{ trans('cruds.customerService.fields.kode_cs') }}</label>
                <input class="form-control {{ $errors->has('kode_cs') ? 'is-invalid' : '' }}" type="text" name="kode_cs" id="kode_cs" value="{{ old('kode_cs', '') }}" required>
