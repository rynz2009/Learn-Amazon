...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.detailTransaksiAff.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.detail-transaksi-affs.update", [$detailTransaksiAff->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_transaksi') ? 'is-invalid' : '' }}" name="kode_transaksi_id" id="kode_transaksi_id" required>
                    @foreach($kode_transaksis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kode_transaksi_id') ? old('kode_transaksi_id') : $detailTransaksiAff->kode_transaksi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah') ? 'is-invalid' : '' }}" type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $detailTransaksiAff->jumlah) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ukuran') ? 'is-invalid' : '' }}" type="text" name="ukuran" id="ukuran" value="{{ old('ukuran', $detailTransaksiAff->ukuran) }}" required>
