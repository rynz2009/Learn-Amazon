...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.list-products.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="kode_produk">{{ trans('cruds.listProduct.fields.kode_produk') }}</label>
                <input class="form-control {{ $errors->has('kode_produk') ? 'is-invalid' : '' }}" type="text" name="kode_produk" id="kode_produk" value="{{ old('kode_produk', 'Kode Product') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('niches') ? 'is-invalid' : '' }}" name="niches[]" id="niches" multiple required>
                    @foreach($niches as $id => $niche)
                        <option value="{{ $id }}" {{ in_array($id, old('niches', [])) ? 'selected' : '' }}>{{ $niche }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link_shopee') ? 'is-invalid' : '' }}" type="text" name="link_shopee" id="link_shopee" value="{{ old('link_shopee', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link_form') ? 'is-invalid' : '' }}" type="text" name="link_form" id="link_form" value="{{ old('link_form', '') }}">
