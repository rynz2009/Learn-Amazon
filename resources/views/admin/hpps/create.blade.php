...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.hpp.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.hpps.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="kode_hpp">{{ trans('cruds.hpp.fields.kode_hpp') }}</label>
                <input class="form-control {{ $errors->has('kode_hpp') ? 'is-invalid' : '' }}" type="text" name="kode_hpp" id="kode_hpp" value="{{ old('kode_hpp', '') }}" required>
