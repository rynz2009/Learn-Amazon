    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $user->nama) }}" required>
                @if($errors->has('nama'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nama') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_rekening') ? 'is-invalid' : '' }}" type="text" name="no_rekening" id="no_rekening" value="{{ old('no_rekening', $user->no_rekening) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_bank') ? 'is-invalid' : '' }}" type="text" name="nama_bank" id="nama_bank" value="{{ old('nama_bank', $user->nama_bank) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_cs') ? 'is-invalid' : '' }}" name="kode_cs_id" id="kode_cs_id">
                    @foreach($kode_cs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kode_cs_id') ? old('kode_cs_id') : $user->kode_cs->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
