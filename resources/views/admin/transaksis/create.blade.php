    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transaksis.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_cs') ? 'is-invalid' : '' }}" name="kode_cs[]" id="kode_cs" multiple required>
                    @foreach($kode_cs as $id => $kode_cs)
                        <option value="{{ $id }}" {{ in_array($id, old('kode_cs', [])) ? 'selected' : '' }}>{{ $kode_cs }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('propinsi') ? 'is-invalid' : '' }}" type="text" name="propinsi" id="propinsi" value="{{ old('propinsi', '') }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Transaksi::PEMBAYARAN_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('pembayaran') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="pembayaran_{{ $key }}" name="pembayaran" value="{{ $key }}" {{ old('pembayaran', 'COD ( Cash On Delivery )') === (string) $key ? 'checked' : '' }} required>
