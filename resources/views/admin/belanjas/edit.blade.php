...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.belanja.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.belanjas.update", [$belanja->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="tanggal">{{ trans('cruds.belanja.fields.tanggal') }}</label>
                <input class="form-control date {{ $errors->has('tanggal') ? 'is-invalid' : '' }}" type="text" name="tanggal" id="tanggal" value="{{ old('tanggal', $belanja->tanggal) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_hpps') ? 'is-invalid' : '' }}" name="kode_hpps[]" id="kode_hpps" multiple>
                    @foreach($kode_hpps as $id => $kode_hpp)
                        <option value="{{ $id }}" {{ (in_array($id, old('kode_hpps', [])) || $belanja->kode_hpps->contains($id)) ? 'selected' : '' }}>{{ $kode_hpp }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_barang') ? 'is-invalid' : '' }}" type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang', $belanja->nama_barang) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah_barang') ? 'is-invalid' : '' }}" type="number" name="jumlah_barang" id="jumlah_barang" value="{{ old('jumlah_barang', $belanja->jumlah_barang) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('harga') ? 'is-invalid' : '' }}" type="number" name="harga" id="harga" value="{{ old('harga', $belanja->harga) }}" step="0.01" required>
                @if($errors->has('harga'))
                    <div class="invalid-feedback">
                        {{ $errors->first('harga') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" name="keterangan" id="keterangan">{{ old('keterangan', $belanja->keterangan) }}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Belanja::SUMBER_BAYAR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('sumber_bayar') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="sumber_bayar_{{ $key }}" name="sumber_bayar" value="{{ $key }}" {{ old('sumber_bayar', $belanja->sumber_bayar) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Belanja::TYPE_BELANJA_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type_belanja') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_belanja_{{ $key }}" name="type_belanja" value="{{ $key }}" {{ old('type_belanja', $belanja->type_belanja) === (string) $key ? 'checked' : '' }} required>
